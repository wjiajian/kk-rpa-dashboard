...
| 业务 inputs、download_dir | 原运行最终保存的参数；不重算日期，不读取任务后来编辑的新值。业务身份由原凭据校验，Profile 按控制台 Run ID 隔离，无需账号别名 |
...
同一失败原因下，不原样重复已经失败且没有新证据支持的操作。可以依据新的观察调整恢复方式；同一 Run 最多分配 3 轮完整接管，轮内可有多次模型请求及工具调用，仍受剩余 900 秒累计预算约束。不增加业务写入补偿。
...
| 提交恢复与续跑 | 本轮最终 summary、from_step、可选 step_result、可选 locator_overrides | 先释放恢复上下文，再调用原 resume；返回接受情况与新尝试关联，之后由执行事件报告进度与终态 |
| 报告无法恢复 | 本轮最终 summary、具体原因、已尝试事项及证据引用 | 发起接管收尾；不直接改写业务成功状态 |
...
**轮数与展示**：每次分配 Agent 开始一轮完整接管，最多 3 轮；重新分配、重启和断连不重置已用轮数。执行过程按时间降序展示，所有 Agent 轮次合并在一条可展开日志中，显示简短动作状态及 `resume` 或 `give_up` 提交的最终总结；长内容内部滚动，不显示工具参数、DOM 或元素解析结果。正常步骤校验通过后和失败收尾前立即上传脱敏截图，接管观察也上传截图；DOM 读取失败不丢弃已取得的图片。每个 Run 仅保留最新一张截图，新图保存成功后删除旧图记录和文件，服务启动时清理历史多余截图；截图失败保留上一张可用图片。
