...
**轮数与展示**：每次分配 Agent 开始一轮完整接管，最多 3 轮；重新分配、重启和断连不重置已用轮数。执行过程按时间降序展示，Agent 每轮只显示 `resume` 或 `give_up` 提交的最终总结，不逐条展示工具返回。正常步骤校验通过后和失败收尾前立即上传脱敏截图，接管观察也保留截图；DOM 读取失败不丢弃已取得的图片。
