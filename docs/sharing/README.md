...
更新日期：2026-09-08。适合研发、业务和运维共同交流，也可按本文顺序组织约 20 分钟的技术分享。
...
| 文档 | 主要内容 | 适合读者 |
| --- | --- | --- |
| [完整平台实施方案](../platform-implementation.md) | 已确认决策、跨仓库数据与接口、四模块设计、五阶段验收及最新实施记录 | 产品、研发、测试、运维 |
...
下一阶段的已确认决策、接口、实施顺序与验收只在 [完整平台实施方案](../platform-implementation.md) 维护；第一版源码导入仅支持 Git。
...
图中表示上述基线已经接入的运行链路。新增平台能力的设计及验收见统一实施方案。
...
| 统一控制台 | 真实总览、应用列表、任务提交、机器人管理、排队、停止、重跑、日志、截图和 Token | 最近 500 次运行统计；最新版本现场完整验收待完成 |
| 失败接管 | 六种受限工具、同一 Run 内续跑、900 秒累计预算、最多 3 轮 | Windows 与真实模型的端到端效果待验收 |
| 参数表单 | Dashboard 已能按部署 Schema 渲染 | 两个应用缺声明，执行端未上报，仍需打通 |
| 后续产品能力 | 可复用计划、定时触发、应用导入与发布 | 尚未实现；历史演示页不再挂载 |
| 部署 | 三服务 Compose、Mac 测试辅助脚本、Windows 启动脚本 | Linux 正式环境验收待完成 |

这里的“已实现”表示存在对应代码。文档同时引用已有的离线验证和 macOS 业务验收记录；它们的范围分别列在两篇说明中。2026-09-08 核对的代码基线为 Dashboard `a7b0853`、Monorepo `b92df4c`；工作区后续改动、目标机器更新情况与新阶段验收不包含在这份基线结论中。
...
| 当前界面与历史演示源码 | 开发、测试和联调统一使用真实控制台，不再切换演示模式或独立测试页 |
