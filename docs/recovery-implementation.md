...
以下首轮记录保留当时的验证范围，不代表最新代码全量验收。

首轮检查使用离线应用上下文、模拟 WebSocket 执行消息、本地 Responses 流式服务和临时 PostgreSQL 17；未登录业务网站、未调用真实模型、未下载业务报表。
...
| 飞书真实企业登录 | 首轮离线验证未覆盖；完整登录与角色现场验收待确认 |
| DeepSeek 官方视觉与 Windows 真实接管 | 首轮离线验证未覆盖；后续已有现场调试，最新完整验收待完成 |
...
- 统一控制台已接入真实总览、应用列表、运行提交、停止重跑、日志、截图、Token 和机器人管理，不再使用独立测试页。该项已有 21 项测试及构建通过记录。
- 两个业务程序已按用户要求取消预期可见身份匹配，保留登录会话与登录失败检查；控制台仅填写账号密码。相关 101 项测试通过，最新行为待真实回归。
- 现场日志曾出现 DOM 观察异常、未携带修正参数续跑及模型输出截断。空 XPath/iframe 观察、无目标等待、截断续接和 Token 上报已有修复；已有现场调试不等于最新完整流程通过。
- Token 优化已有上述离线体积对比，实际用量、费用和恢复成功率待新运行记录确认。
