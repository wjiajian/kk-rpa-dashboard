...
执行过程按事件时间降序展示，同一时间按序号降序。正常步骤通过原校验后和失败收尾前采集脱敏截图，立即发送独立证据事件；接管观察的截图随操作结果回传，DOM 读取失败仍可上传截图。服务端保存后提供鉴权图片地址，补传同一事件不会生成重复附件，截图失败原因写入事件。

每次分配 Agent 开始一轮完整接管，同一个 Run 最多 3 轮；轮内可有多次模型请求与工具调用。累计轮数由服务端持久化，重新分配、断连和重启不重置。Agent 通过 `resume` 或 `give_up` 的必填 `summary` 提交本轮最终总结，执行过程每轮只显示这条总结，不展示逐条工具结果；中断或超时时由服务端记录结束原因。第 3 轮交回程序后仍可正常执行，若再失败则结束接管。900 秒累计接管时限及程序阶段暂停计时的规则保持不变。
