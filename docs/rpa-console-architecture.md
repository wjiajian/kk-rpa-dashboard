...
初稿：2026-09-05；进度更新：2026-09-08

状态：历史设计背景。文首状态表保留 Dashboard `a7b0853`、Monorepo `b92df4c` 基线；工作区后续进展与验收统一见 [完整平台实施方案](platform-implementation.md)。

后续实施以 [完整平台实施方案](platform-implementation.md) 为准。本文保留初稿背景：ZIP 导入不进入首版，重跑沿用原运行凭据，计划与临时运行并存；下文与这些新决策不同的内容不再作为实施要求。

## 设计基线状态（2026-09-08）

| 范围 | 当前状态 |
| --- | --- |
| Web/API/Agent 与机器人连接 | 已实现三服务、飞书会话与角色接口、Windows 主动 WSS、独立应用 Worker、真实运行队列和失败接管 |
| 统一界面 | 总览、应用列表、任务提交、运行详情及机器人管理均接真实接口；不再切换演示/测试页，也不回退样例数据 |
| 应用与参数 | 应用列表聚合机器人已部署版本。前端支持 input_schema 或 form_schema.properties.inputs，但两个应用缺静态声明，执行端仅上报 app_id/version |
| 任务与触发 | 新建任务直接创建 Run；任务列表实际读取运行。Task 保存、编辑、启停、Cron 到期触发和相对日期解析尚未实现 |
| 执行框架 | 公共 execute_application、中文事件、协作停止、截图、原 verify 续跑与收尾确认已实现 |
| 发布与设置 | Git/ZIP 导入、自动安装分发回滚、管理员及保留期设置页面尚未接入；目前使用本机部署配置和服务端配置 |
| 验收 | 已有离线、历史 macOS 业务验收和现场调试记录；最新 Windows + Dashboard + 模型完整回归及 Linux 正式部署验收待完成 |

当前填写顺序为“应用与机器人 → 参数弹窗 → 登录配置 → 触发配置”，初始不预选应用、机器人或业务参数。账号、密码由用户输入；两个程序已取消预期可见身份匹配。当前只有手动执行，后续保存计划时再落实下文的可复用配置语义。
...
当前已提供 `rpa_core.cli.execute_application` 公共运行入口，执行桥复用应用配置、浏览器生命周期、Runner 和失败记录。以下三项为原接入要求，现已实现。
...
进度更新：上述公共运行入口、中文事件、协作停止与截图已通过执行桥接入；控制台运行状态由后端维护，核心本地记录和控制台 Run 仍是不同对象。实现与验证见 [接管实施说明](recovery-implementation.md)。
...
以下是原方案接口清单，不等同于当前路由表。登录、身份、运行读取、停止、重跑、业务事件、截图和机器人列表已实现；直接创建运行使用 `POST /api/runs`，取消排队共用 stop 接口。导入、应用目录、Task、管理员及设置接口仍未实现，机器人队列没有独立接口。实际接口见 `apps/backend/src/rpa_console/api.py`。
...
统一维护于 [完整平台实施方案](platform-implementation.md#7-阶段交付与验收记录)。本文不再保留另一份阶段待办；已实现状态见文首，历史设计用于理解背景。
