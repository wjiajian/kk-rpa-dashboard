...
`BACKEND_URL=http://backend:8000`、`EVIDENCE_DIR=/data/evidence`、`PI_SESSION_DIR=/data/sessions` 是 Compose 固定的容器内地址与路径，已在示例注释中列明，不是另外三项待填参数。Windows 只配置控制台地址和机器人连接凭据。业务账号和密码在控制台发起运行时填写，加密存入独立凭据表，不写入 `.env`、运行快照或日志。
