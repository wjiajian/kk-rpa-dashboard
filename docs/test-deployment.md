...
应用路径、版本、模块名和 WSS 地址由现有模板提供，不用手填 TOML。相对路径以配置文件所在目录为基准，不受 PowerShell 当前目录影响。账号别名在控制台填写，使用大写字母开头的稳定别名，例如 `STORE_001`，Windows 不需要预先登记账号。
...
Windows 在线后，在控制台的真实运行列表选择“发起应用运行”，填写运行名称、机器人、已部署应用版本和账号别名，再填写业务登录账号、密码、登录后预期可见身份，以及业务输入与下载目录。预期身份用于原应用的登录校验。
...
| 4 | 正常 Run | 填写账号别名、登录账号、密码及预期身份，按应用要求填输入；步骤、下载及结束状态正确 |
