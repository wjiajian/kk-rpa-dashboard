...
状态：保留初稿的完整前端设计，下文的拟增目录、页面和接口不作为实现清单。当前已统一使用真实控制台，详见 [Dashboard 当前进度](sharing/dashboard.md#8-已实现与待实现清单)。

当前 `App.tsx` 挂载 `live/` 下的真实总览、应用列表、任务提交、运行记录、日志截图与机器人页面。开发与联调使用同一界面，接口失败显示错误，不回退模拟数据；历史 `pages/`、`mock/` 不再挂载。新建任务按“应用与机器人 → 参数弹窗 → 登录配置 → 触发配置”填写，初始不预选或填入默认业务参数，账号密码由用户输入。

动态表单已支持机器人部署中的 Schema；两个程序的静态声明与执行端上报尚未完成，无声明时显示 JSON 输入。提交直接创建真实 Run，当前任务列表也是运行列表；保存可复用计划、定时触发、导入发布和管理设置尚未接入。
...
- 详情页自上而下：
  ...
  2. 信息区：应用版本、机器人、触发来源、Agent Token 用量及实际业务参数（日期已是解析后值），不显示来源运行链接。Token 显示整次运行已记录总量、输入（含缓存）、输出及缓存命中；缺失用量显示“尚未记录”，部分响应未返回用量时明确提示。运行表单无需账号别名。同 Run 最多 3 轮 Agent 接管、累计 900 秒。
  3. 尝试明细：每次 `ExecutionAttempt` 一行（第几次、本地 run_id、结果、耗时），Agent 接管产生的续跑尝试归入同一运行下（架构 §3.1）。
  4. 日志区：§6 的 SSE 实时日志，按时间降序排列。所有接管轮次合并成一条默认折叠的“Agent 执行情况”，按最新进展排序；点击展开后显示简短动作状态和各轮最终总结，不显示工具参数、DOM 或元素解析结果，内容超过 320px 时内部滚动。
  5. 截图区：随执行端证据事件更新，只显示最新一张现场截图，点击放大。后端在新图保存成功后删除旧图记录和文件；截图失败时保留上一张可用图片。
