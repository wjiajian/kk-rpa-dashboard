...
describe("统一控制台路由", () => {
  ...
  it("计划列表和保存入口使用真实计划接口", () => {
    const html = page("/tasks");
    expect(html).toContain("接口计划");
    expect(html).toContain("/tasks/saved-plan");
    const create = page("/tasks/new");
    expect(create).toContain("保存计划");
    expect(create).toContain("启用定时");
    expect(create).not.toContain("立即执行");
  });
  it("只读成员不出现管理导航或新建入口", () => {
